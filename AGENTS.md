...
· `/checkers` · `/catan` · `/sudoku` · `/dark-dungeon` · `/neon-trail` · `/block-tower` · `/force-deflector` · `/volt-runner` · `/hill-drive` · `/beat-maker` · `/fable-world` · `/profile` · `/whats-new`
...
- **Катан** (`/catan`): готовый Hexland-билд iframe'ом. **Катан Fable** (`/catan-fable`) сохраняет standalone-режим «База/Мореходы» и также имеет сетевые комнаты на 4 места. Frontend: `features/catanFable/CatanFableScreen.tsx`; backend: `routes/catan.ts`, `socket/catan.ts`, `services/catanService.ts`, чистый движок в `backend/src/catan/`. Хозяин может заполнить незанятые места ботами; сетевая партия серверная и хранится в Postgres.
...
- **Jedi Survivors** (`/jedi-survivors`): standalone выживач в духе Vampire Survivors, один файл canvas+JS, iframe `public/games/jedi-survivors/index.html` (тот же паттерн — `TopNav` + `.force-deflector-frame-shell`, экран `features/jediSurvivors/JediSurvivorsScreen.tsx`). Источник — `apps/jedi-survivors/index.html` (копируй оттуда при обновлении). 20 базовых оружий + 20 эволюций + 10 пассивок, 9 героев (включая секретного), 16 аркан и 4 этапа; волновой спавн по минутам, боссы и мета-магазин «Академия джедаев» в localStorage. Управление — джойстик пальцем/WASD, короткий тап — точка «идти сюда». Backend не нужен. Перед выбором следующей задачи читай `JEDI_SURVIVORS_ROADMAP.md`; при завершении обновляй чекбоксы и оба changelog.
...
- **NEON TRAIL** (`/neon-trail`): standalone-головоломка одной линией в `frontend/public/games/neon-trail/`, экран `features/neonTrail/NeonTrailScreen.tsx`. 18 уровней, управление свайпом/мышью/клавиатурой, подсказки и локальный прогресс `neon-trail-progress-v1`; backend не нужен. Источник — `apps/neon-trail-game/` в workspace, при обновлении синхронизируй `index.html`, `styles.css` и `src/`.
...
- **BurpiOpus** (`/burpi-opus`): дневник тренировок (бёрпи), а не игра. Standalone-приложение без бандлера в `frontend/public/games/burpi-opus/`, экран-оболочка `features/burpiOpus/BurpiOpusScreen.tsx` (full-bleed iframe, класс `.burpi-opus-screen`). **Source of truth — `apps/burpi-opus/` в корне воркспейса**, публикация только через `node apps/burpi-opus/sync.mjs`: скрипт переносит файлы и дописывает `?v=<версия>` ко всем импортам, иначе WebView Telegram смешивает старые и новые модули. Версия берётся из `src/whats-new.js` — поднимай её при каждом выпуске. Оболочка отвечает ровно за три вещи, недоступные вложенному документу: пробрасывает хаптику Telegram, шлёт внутрь безопасные отступы (`safe-area`) и обрабатывает выход в хаб (`postMessage {source:"burpi-opus", type:"exit"}`); полноэкранный режим включается на входе и обязательно выключается на выходе. Число подходов НЕ ограничено: разбивка — только подсказка, задание закрывается по сумме повторов (`goalReached`). Закончил раньше — тренировка идёт в дневник и в серию, но цель не засчитывается и не растёт. Незакрытая работа за день переносится в следующий заход того же дня (`carryOverToday`). Цель «РЕКОРДСМЕНА» растёт от числа ВЗЯТЫХ целей этого уровня, а не от календаря — пропуск дня не раздувает план. Личный рекорд — лучший день, а не лучший подход. Данные лежат в localStorage `burpi-opus.v1`, backend не нужен.
- **WAVE FORGE** (`/beat-maker`): бит-мейкер в вейв/8-бит стиле, standalone iframe `public/games/beat-maker/`, экран `features/beatMaker/BeatMakerScreen.tsx`. Пошаговый секвенсор: барабаны (kick/snare/hat/clap) + 3 синта (bass/lead/arp) с пиано-роллом, привязанным к ладу; свинг/темп/октавы/биткрашер; кнопка «Сгенерить». Движок `beat-maker/synth-core.js` — чистый JS без DOM/WebAudio, общий для браузера и `beat-maker/render.mjs` (Node-рендер кода `WF1:…`/JSON в WAV — так агент достаёт треки пользователя); воспроизведение, WAV-экспорт и Node-рендер идентичны до сэмпла. Сохранение: localStorage + синк в аккаунт через postMessage-мост (React-шелл) → **backend роут `/api/beats`** (GET/POST/DELETE, authed, модель `BeatTrack`, upsert по имени, лимит 60). Без бэкенда апка работает на localStorage + экспорт кода/WAV. Экран «Что нового» встроен в саму апку.
...
- **Что нового — обязательно при каждом деплое**: перед публикацией добавляй новую верхнюю запись в `frontend/src/data/changelog.ts` с точными датой и временем, коротким заголовком и 1–3 пунктами о том, что вошло в выпуск. Она автоматически видна в разделе «Что нового» внутри GamePass.
  **Время брать по местному времени машины** — из `date '+%Y-%m-%d %H:%M %Z'` — и подписывать зону в скобках. Город не зашивать: раньше правило требовало «по Алматы», и после переезда владельца на Кипр записи уезжали на два часа вперёд, а после полуночи — ещё и на сутки.
...
- **Jedi Survivors**: продуктовый план и порядок задач находятся в `JEDI_SURVIVORS_ROADMAP.md`. Ближайшие приоритеты: реликвии/гробы на карте, торговец в забеге и Limit Break.

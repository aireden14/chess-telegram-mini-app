...
· `/checkers` · `/catan` · `/sudoku` · `/force-deflector` · `/volt-runner` · `/fable-world` · `/profile` · `/whats-new`
...
- **VOLT RUNNER** (`/volt-runner`): standalone скоростной платформер в `frontend/public/games/volt-runner/index.html`, экран `features/voltRunner/VoltRunnerScreen.tsx`. 6 авторских неоновых трасс, двойной прыжок, рывок, пружины, дроны, лазеры, чекпоинты и ранги; canvas-графика и WebAudio полностью процедурные. Прогресс в `gamepass.volt-runner.v1`, backend не нужен. Landscape/full-bleed. Перед следующими правками читай `VOLT_RUNNER_ROADMAP.md`; выполняй релизы по порядку и обновляй чекбоксы вместе с обоими changelog.

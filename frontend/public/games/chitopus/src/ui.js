...
const ICONS = {
  ...
  books:
    '<path d="M4.6 5.4c2.6-1.1 4.8-1.1 7.4.5 2.6-1.6 4.8-1.6 7.4-.5v12.9c-2.6-1.1-4.8-1.1-7.4.5-2.6-1.6-4.8-1.6-7.4-.5V5.4Zm7.4.5v12.9" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>',
  diary:
...
/**
 * Шкала 1…10 одной строкой. Ряд листается вбок на узких экранах, а не
 * разваливается на два — иначе «в начале» и «в конце» перестают читаться как пара.
 */
export function scaleRow(value, onChange) {
  const wrap = h("div.scale");
  const paint = (v) => {
    wrap.querySelectorAll(".scale-dot").forEach((n, i) => {
      n.classList.toggle("is-on", i + 1 <= v);
      n.classList.toggle("is-pick", i + 1 === v);
    });
  };
  for (let i = 1; i <= 10; i += 1) {
    wrap.append(h("button.btn.scale-dot", {
      type: "button",
      text: String(i),
      "aria-label": `${i} из 10`,
      on: {
...
export function starsRow(value, onChange) {
  const wrap = h("div.stars");
  const paint = (v) => {
    wrap.querySelectorAll(".star").forEach((n, i) => n.classList.toggle("is-on", i + 1 <= v));
  };
  for (let i = 1; i <= 5; i += 1) {
    wrap.append(h("button.btn.star", {
      type: "button",
      text: "★",
      "aria-label": `${i} из 5`,
      on: {
